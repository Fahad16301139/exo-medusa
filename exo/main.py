...
def configure_uvloop():
  ...
  # Only use uvloop if available (not on Windows)
  try:
    import uvloop
    uvloop.install()
    print("Using uvloop")
...
parser.add_argument("--inference-engine", type=str, default="torch", help="Inference engine to use")
...
parser.add_argument("--medusa-enabled", action="store_true", help="Enable Medusa parallel decoding")
parser.add_argument("--medusa-num-heads", type=int, default=5, help="Number of Medusa heads to use")
parser.add_argument("--medusa-num-layers", type=int, default=1, help="Number of Medusa layers to use")
...
if args.inference_engine == "torch":
    from exo.inference.torch import TorchInferenceEngine
    inference_engine = TorchInferenceEngine(
        shard_downloader=shard_downloader,
        medusa_enabled=args.medusa_enabled,
...
async def run_model_cli(node: Node, model_name: str, prompt: str):
  ...
  repo_id = get_repo(shard.model_id, inference_class)
  if repo_id is None:
    print(f"Error: No repository found for model '{model_name}' with inference engine {inference_class}")
    return
...
async def eval_model_cli(node: Node, model_name: str, dataloader, batch_size):
  inference_class = node.inference_engine.__class__.__name__
  shard = build_base_shard(model_name, inference_class)
  if not shard:
    print(f"Error: Unsupported model '{model_name}' for inference engine {inference_class}")
