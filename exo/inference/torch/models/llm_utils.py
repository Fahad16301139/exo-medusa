...
class ShardTransformerDecoder(TransformerDecoder):
  ...
  def setup_caches(
    ...
    *,
    ...
    if not hasattr(self, 'encoder_max_cache_seq_len'):
      self.encoder_max_cache_seq_len = None

    # Get non-None layers only
    valid_layers = [layer for layer in self.layers if layer is not None]
    if not valid_layers:
      if DEBUG >= 1:
        print("No valid layers found for cache setup")
      return

    # Try to set up caches for each valid layer
    for layer in valid_layers:
      try:
        # Try the function with keyword arguments
        layer.setup_caches(
        ...
        if DEBUG >= 2:
          print(f"Layer setup_caches with kwargs failed: {e}, trying alternatives")
        
        try:
          # Try with positional arguments
          if hasattr(layer, 'setup_caches'):
            if self.encoder_max_cache_seq_len is not None:
              layer.setup_caches(batch_size, dtype, self.encoder_max_cache_seq_len, self.decoder_max_cache_seq_len)
            else:
              layer.setup_caches(batch_size, dtype, decoder_max_seq_len=self.decoder_max_cache_seq_len)
        except Exception as inner_e:
          if DEBUG >= 1:
            print(f"Error setting up caches for layer: {inner_e}")
    ...
    for layer in self.layers:
      if layer is not None:
        if hasattr(layer, 'reset_cache'):
          layer.reset_cache()
        elif hasattr(layer, 'reset_caches'):
          layer.reset_caches()
