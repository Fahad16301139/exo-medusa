...
class ShardedGeneralModel(nn.Module):
  def __init__(
    ...
    # Add decoder_max_cache_seq_len attribute with default value
  ...
  def caches_are_enabled(self):
    """Check if caches are enabled for this model."""
    if hasattr(self.model, 'caches_are_enabled'):
      return self.model.caches_are_enabled()
    return self.use_cache
    
  def setup_caches(self, batch_size=1, dtype=torch.float16, decoder_max_seq_len=1024):
    """Set up caches for this model."""
    # Update our own attribute first
    if decoder_max_seq_len is not None:
      self.decoder_max_cache_seq_len = decoder_max_seq_len
    
    if hasattr(self.model, 'setup_caches'):
      # Pass parameters as keyword arguments to avoid position issues
      return self.model.setup_caches(
        batch_size=batch_size,
        dtype=dtype,
        decoder_max_seq_len=decoder_max_seq_len
      )
    if DEBUG >= 1:
      print("setup_caches called but model doesn't support it")
    
  def reset_caches(self):
    """Reset caches for this model."""
    if hasattr(self.model, 'reset_caches'):
      return self.model.reset_caches()
    elif hasattr(self.model, 'reset_cache'):
      return self.model.reset_cache()
    if DEBUG >= 1:
      print("reset_caches called but model doesn't support it")
