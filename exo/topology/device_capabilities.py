...
CHIP_FLOPS = {
  ...
  "NVIDIA GEFORCE RTX 4060": DeviceFlops(fp32=15.0*TFLOPS, fp16=30.0*TFLOPS, int8=60.0*TFLOPS),
  "NVIDIA GEFORCE RTX 4050": DeviceFlops(fp32=10.6*TFLOPS, fp16=21.2*TFLOPS, int8=42.4*TFLOPS),
  "NVIDIA GEFORCE RTX 4050 LAPTOP GPU": DeviceFlops(fp32=10.6*TFLOPS, fp16=21.2*TFLOPS, int8=42.4*TFLOPS),
  # RTX 30 series
...
async def windows_device_capabilities() -> DeviceCapabilities:
  ...
  if DEBUG >= 2: print(f"Found GPUs: {gpu_names}")
  ...
  if contains_nvidia:
    try:
      ...
      # Try common variant names if exact match not found
      if flops is None:
        for key in CHIP_FLOPS:
          if gpu_name in key or key in gpu_name:
            flops = CHIP_FLOPS[key]
            if DEBUG >= 1: print(f"Found similar GPU model: {key}")
            break
      
      # Assign reasonable defaults based on memory tier if still not found
      if flops is None:
        memory_gb = gpu_memory_info.total / (1024**3)
        if DEBUG >= 1: print(f"No exact match found. Using tier-based defaults. Memory: {memory_gb:.2f} GB")
        
        if memory_gb >= 16:
          flops = DeviceFlops(fp32=30.0*TFLOPS, fp16=60.0*TFLOPS, int8=120.0*TFLOPS)
        elif memory_gb >= 8:
          flops = DeviceFlops(fp32=15.0*TFLOPS, fp16=30.0*TFLOPS, int8=60.0*TFLOPS)
        elif memory_gb >= 6:
          flops = DeviceFlops(fp32=10.0*TFLOPS, fp16=20.0*TFLOPS, int8=40.0*TFLOPS)
        elif memory_gb >= 4:
          flops = DeviceFlops(fp32=7.0*TFLOPS, fp16=14.0*TFLOPS, int8=28.0*TFLOPS)
        else:
          flops = DeviceFlops(fp32=5.0*TFLOPS, fp16=10.0*TFLOPS, int8=20.0*TFLOPS)
      ...
      if DEBUG >= 1: print(f"Error detecting NVIDIA GPU capabilities: {e}")
    ...
    try:
      ...
      # Assign reasonable defaults based on found GPU model name
      if flops is None:
        # Default TFLOPS values for common AMD GPU families
        if "RX 7900" in gpu_name or "RX 6900" in gpu_name:
          flops = DeviceFlops(fp32=50.0*TFLOPS, fp16=100.0*TFLOPS, int8=200.0*TFLOPS)
        elif "RX 7800" in gpu_name or "RX 6800" in gpu_name:
          flops = DeviceFlops(fp32=30.0*TFLOPS, fp16=60.0*TFLOPS, int8=120.0*TFLOPS)
        elif "RX 7700" in gpu_name or "RX 6700" in gpu_name:
          flops = DeviceFlops(fp32=20.0*TFLOPS, fp16=40.0*TFLOPS, int8=80.0*TFLOPS)
        elif "RX 7600" in gpu_name or "RX 6600" in gpu_name:
          flops = DeviceFlops(fp32=15.0*TFLOPS, fp16=30.0*TFLOPS, int8=60.0*TFLOPS)
        else:
          flops = DeviceFlops(fp32=10.0*TFLOPS, fp16=20.0*TFLOPS, int8=40.0*TFLOPS)
      ...
      if DEBUG >= 1: print(f"Error detecting AMD GPU capabilities: {e}")
      # Fall through to generic detection

  # Generic GPU detection based on name matching
  for gpu_name in gpu_names:
    if DEBUG >= 1: print(f"Trying to match GPU: {gpu_name}")
    # Try to extract known GPU model names from the full display name
    for key in CHIP_FLOPS:
      if key.upper() in gpu_name.upper():
        if DEBUG >= 1: print(f"Found match: {key}")
        # Use estimated memory as we can't reliably get actual VRAM
        estimated_memory = 8192  # Default to 8GB
        for gpu in gpus_info:
          if gpu['Name'] == gpu_name and gpu['AdapterRAM'] > 0:
            estimated_memory = gpu['AdapterRAM'] // 2**20
    ...
    # Check for model number patterns in the GPU name
    if "nvidia" in gpu_name.lower() or "geforce" in gpu_name.lower():
      if "rtx" in gpu_name.lower():
        # Match RTX series
        for series in ["4090", "4080", "4070", "4060", "3090", "3080", "3070", "3060", "2080", "2070", "2060"]:
          if series in gpu_name:
            # Try to match this model specifically
            for key in CHIP_FLOPS:
              if series in key and "nvidia" in key.lower():
                if DEBUG >= 1: print(f"Found NVIDIA match by series: {key}")
                return DeviceCapabilities(
                  model=f"Windows Box ({gpu_name})",
                  chip=key,
                  memory=psutil.virtual_memory().total // 2**20 // 4,  # Estimate 1/4 of system RAM
                  flops=CHIP_FLOPS[key],
                )
        
        # Default RTX TFLOPS if series not identified
        return DeviceCapabilities(
          model=f"Windows Box ({gpu_name})",
          chip=gpu_name,
          memory=psutil.virtual_memory().total // 2**20 // 4,
          flops=DeviceFlops(fp32=12.0*TFLOPS, fp16=24.0*TFLOPS, int8=48.0*TFLOPS),
        )
    
    elif "amd" in gpu_name.lower() or "radeon" in gpu_name.lower():
      # Match AMD series
      for series in ["7900", "7800", "7700", "7600", "6900", "6800", "6700", "6600"]:
        if series in gpu_name:
          for key in CHIP_FLOPS:
            if series in key and "amd" in key.lower():
              if DEBUG >= 1: print(f"Found AMD match by series: {key}")
              return DeviceCapabilities(
                model=f"Windows Box ({gpu_name})",
                chip=key,
                memory=psutil.virtual_memory().total // 2**20 // 4,
      ...
      # Default AMD TFLOPS if series not identified
      return DeviceCapabilities(
        model=f"Windows Box ({gpu_name})",
        chip=gpu_name,
        memory=psutil.virtual_memory().total // 2**20 // 4,
  ...
  # No GPU detected or failed to match, use system memory
  return DeviceCapabilities(
    model=f"Windows Box (Device: Unknown)",
    chip=f"Unknown Chip (Device(s): {gpu_names})",
    memory=psutil.virtual_memory().total // 2**20,
    flops=DeviceFlops(fp32=5.0*TFLOPS, fp16=10.0*TFLOPS, int8=20.0*TFLOPS),  # Default to reasonable values
  )
