...
class ChatGPTAPI:
  ...
  async def handle_post_chat_token_encode(self, request):
    ...
    if repo_id is None:
      ...
      return web.json_response(
        {"detail": f"No repository found for model: {model} with inference engine {self.inference_engine_classname}. Supported models for this engine: {supported_models}"},
        status=400,
      )
  ...
  async def handle_post_chat_completions(self, request):
    ...
    if repo_id is None:
      supported_models = [model for model, info in model_cards.items() if self.inference_engine_classname in info.get("repo", {})]
      return web.json_response(
        {"detail": f"No repository found for model: {chat_request.model} with inference engine {self.inference_engine_classname}. Supported models for this engine: {supported_models}"},
        status=400,
      )
